...
const Payments = () => {
    ...
    return (
        <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
            <h2 style={{ marginBottom: '20px', color: '#333' }}>{t('payment_history_title') || "Payment History & Invoices"}</h2>
            ...
            <div style={{ background: '#fff', borderRadius: '8px', boxShadow: '0 2px 5px rgba(0,0,0,0.1)', overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                    <thead style={{ background: '#f4f4f4', borderBottom: '2px solid #ddd' }}>
                        <tr>
                            <th style={{ padding: '12px', textAlign: 'left', color: '#555' }}>{t('date_label') || "Date"}</th>
                            <th style={{ padding: '12px', textAlign: 'left', color: '#555' }}>{t('bill_no_label') || "Bill No."}</th>
                            <th style={{ padding: '12px', textAlign: 'left', color: '#555' }}>{t('client_label') || "Client"}</th>
                            <th style={{ padding: '12px', textAlign: 'left', color: '#555' }}>{t('project_label') || "Project"}</th>
                            <th style={{ padding: '12px', textAlign: 'left', color: '#555' }}>{t('amount_label') || "Amount"}</th>
                            <th style={{ padding: '12px', textAlign: 'left', color: '#555' }}>{t('mode_label') || "Mode"}</th>
                            <th style={{ padding: '12px', textAlign: 'center', color: '#555' }}>{t('invoice_label') || "Invoice"}</th>
                            <th style={{ padding: '12px', textAlign: 'center', color: '#555' }}>{t('actions_label') || "Actions"}</th>
                        </tr>
                    </thead>
                    <tbody>
                        {payments.length === 0 ? (
                            <tr>
                                <td colSpan="8" style={{ padding: '20px', textAlign: 'center', color: '#777' }}>
                                    {t('no_payments_msg') || "No payments recorded yet."}
                                </td>
                            </tr>
                        ) : (
                            payments.map((payment) => (
                                <tr key={payment._id} style={{ borderBottom: '1px solid #eee' }}>
                                    ...
                                    <td style={{ padding: '12px', textAlign: 'center' }}>
                                        ...
                                        >
                                            {t('download_pdf_btn') || "Download PDF"}
                                        </a>
                                    </td>
                                    <td style={{ padding: '12px', textAlign: 'center' }}>
