...
export default function UploadRequirementsModal({ isOpen, onClose, requestId, onSuccess }) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!description.trim()) {
            setError(t('error_missing_description') || "Please provide a description.");
            return;
        }
        setLoading(true);
        setError("");
        try {
            ...
            setError(err.response?.data?.message || t('fail_submit_requirements') || "Failed to submit requirements.");
        } finally {
            setLoading(false);
        }
    };
        ...
        <div style={{
            ...
            <div style={{
                ...
                <h2 style={{ margin: "0 0 6px", fontSize: "20px", fontWeight: "700", color: "#1e293b" }}>
                    {t('upload_project_requirements') || "Upload Project Requirements"}
                </h2>
                <p style={{ margin: "0 0 24px", color: "#64748b", fontSize: "14px" }}>
                    {t('upload_requirements_sub') || "Provide details and images for your approved project."}
                </p>

                <form onSubmit={handleSubmit}>
                    <div style={{ marginBottom: "16px" }}>
                        <label style={{ display: "block", fontSize: "13px", fontWeight: "600", color: "#374151", marginBottom: "6px" }}>
                            {t('requirements_description_label') || "Requirements Description *"}
                        </label>
                        <textarea
                            value={description}
                            onChange={e => setDescription(e.target.value)}
                            rows={5}
                            placeholder={t('requirements_desc_placeholder') || "Describe your project requirements in detail..."}
                            style={{
                    ...
                    <div style={{ marginBottom: "24px" }}>
                        <label style={{ display: "block", fontSize: "13px", fontWeight: "600", color: "#374151", marginBottom: "6px" }}>
                            {t('attachment_images_label') || "Attachment Images (optional, max 5)"}
                        </label>
                        <input
                            type="file"
                            accept="image/*"
                        ...
                        {images.length > 0 && (
                            <p style={{ fontSize: "12px", color: "#64748b", marginTop: "4px" }}>
                                {images.length} {t('images_selected') || "image(s) selected"}
                            </p>
                        )}
                    </div>
                    ...
                    <div style={{ display: "flex", gap: "12px" }}>
                        ...
                        >
                            {loading ? (t('submitting') || "Submitting...") : (t('submit_requirements') || "Submit Requirements")}
                        </button>
                        <button
                            type="button"
                            onClick={onClose}
